...
def extraer_S6(df):
    ...
    df["entidadFederativa"] = df.parties.map(lambda x: x.get("address", {}).get("region").get())
    df["parties_address"] = df.parties.map(get_address_string)
    df["parties_roles"] = df.parties.map(lambda x: x.get("roles", []))
    df["rol"] = df.apply(lambda row: diccionario_roles.get(row["parties_roles"][0], "Desconocido") if row["parties_roles"] else "Desconocido", axis=1)

    df = df.drop(columns=["parties"])
    df = df.reset_index(drop = True)
    return df

def agregar_columna_parties_names(df):
    df['parties_names_ocid'] = df.groupby('ocid')['parties_name'].transform(lambda x: ', '.join(x))
    return df
...
for i in range(len(contenido_ruta_bulk_s6)):
    ...
    if os.path.isfile(ruta_bulk_s6_nombres +'/'+contenido_ruta_bulk_s6[i]) == True:
        ...
        #pprint(df.shape)
        df = extraer_S6(df)
        df = agregar_columna_parties_names(df)
